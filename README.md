...
- **The day in the field** at **Admin → Visits → Day view**: one day, rep by rep, *as it actually went* rather than as it was planned. The calls are listed in the order they really happened — a rep who took the third clinic first reads that way — with what is still outstanding at the bottom, so "is the round happening?" has an answer at four in the afternoon.
  ...
  - **Samples given** as units and per product, orders taken, and **how far the rep actually went** — each leg measured between where they checked in, as the crow flies, with the day's total set against the distance the plan intended. A round planned at 43 km and walked in 61 went somewhere it was not meant to; one walked in 12 is a plan mostly not attempted. A leg measured from a clinic's registered address rather than a phone fix is labelled as such, and 0°, 0° is refused outright — taken at face value it puts six thousand kilometres between two clinics on the same street.
...
- **The invoice vault** — every bill the company **receives**, in one place, by month. The other direction from `/admin/billing`, which is money owed *to* the company: this is the purchase paper the GST input credit sits on and the bundle the accountant asks for on the fifth.
  ...
  - **Supplier API keys**, one per vendor under **Super admin → Connections**, stored encrypted and never shown again. Save-and-test proves a key the day it is entered rather than at month end.
  - **Shopify and Shiprocket need nothing entered** — they use the connections the Sales CRM already holds. That is not a convenience for Shopify but a necessity: Shopify no longer issues the pasteable `shpat_` tokens a credential form expects, so the OAuth token the Sales CRM earned is the only one that exists. Fill those fields in only to point the vault at a different account.
  - **What a fetch can honestly bring back, said plainly.** Shiprocket's order invoices come back as Shiprocket's own tax invoice PDFs. Razorpay, Shopify and Meta publish the figures on an API and the invoice only in their dashboard — so a fetch builds a *statement* of every transaction with its fee and tax, which ties to the bank line, and the card goes on asking for the PDF. A statement is not a tax invoice and says so in its own first lines; a fetch that produces one reports as a warning, not a success.
  - **Manual upload for everything else** — rent, the accountant's own fee, a courier's paper bill — under Offline & other. The file is the only required field, because a form that demands a tax figure first is a form that ends with the invoice still in the Downloads folder.
