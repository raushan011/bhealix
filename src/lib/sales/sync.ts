import { SalesOrder, SalesRep, SalesSettings } from "@/models/Sales";
...
import { codesOn, fetchOrders, mapOrder } from "./shopify";
...
async function couponIndex(): Promise<Map<string, { repId: string; suffix: string }>> {
...
export async function syncOrders(options: { since?: Date } = {}): Promise<SyncReport> {
  ...
  for (const raw of orders) {
    ...
    const order = existing.get(mapped.shopifyOrderId);
    if (order) {
      ...
      Object.assign(order, {
        ...
        cancelledAt: mapped.cancelledAt ?? null, fullyRefunded: mapped.fullyRefunded, syncedAt: new Date()
      });
      recalculateCommission(order, rules, { holdDays });
      await order.save();
    } else {
      report.ordersCreated++;
      const created = new SalesOrder({
        source: "Shopify",
        ...
        discountCodes: mapped.discountCodes, items: mapped.items, totals: mapped.totals,
        financialStatus: mapped.financialStatus, paymentMethod: mapped.paymentMethod,
        cancelledAt: mapped.cancelledAt ?? undefined, fullyRefunded: mapped.fullyRefunded, syncedAt: new Date()
      });
      recalculateCommission(created, rules, { holdDays });
      await created.save();
    }
